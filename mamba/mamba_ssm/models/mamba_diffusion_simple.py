...
class Mamba_Diffusion(nn.Module):
    def __init__(
        self,
        d_in_chans: int,
        d_model: int,
        ...
        if True:
            ...
            if self.n_context_token > 0:
                self.context_embed = nn.Linear(
                    self.context_dim, embed_dim, **factory_kwargs
                )
            print("n_context_token", self.n_context_token)
            self.extras = 1 + self.n_context_token

            self.pos_embed = nn.Parameter(
                torch.zeros(1, self.extras + self.n_token, embed_dim, **factory_kwargs)
            )
        ...
        self.decoder_pred = nn.Linear(
            embed_dim, self.patch_dim, bias=True, **factory_kwargs
        )
...
class Mamba_Diffusion_UNet(nn.Module):
    def __init__(
        ...
        self.extras = 1 + self.n_context_token
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.extras + self.n_token, self.embed_dim, **factory_kwargs)
        )
